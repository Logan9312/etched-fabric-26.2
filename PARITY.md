...
| Automated regression suite | automated verified | `build` runs packet/component/resource parity checks plus server GameTests that audit every server mixin, stress 32 villagers through 100 AI ticks, exercise the Bard work-package mixin, place every Etched block/block entity and tick the custom minecart. Client GameTests audit client mixins, complete resource reload and an integrated-world join, render frames, and capture a screenshot on the self-hosted `etched-ci` runners; a second launch smoke-tests the production JAR. Registry-aware ItemStack/payload fixtures and multi-client scenarios remain. |
