...
public class AbstractOnlineSoundInstance extends AbstractSoundInstance {
    ...
    @Override
    public CompletableFuture<AudioStream> getAudioStream(SoundBufferLibrary loader, Identifier soundId, boolean repeatInstantly) {
        if (!(this.sound instanceof OnlineSound onlineSound)) {
            return super.getAudioStream(loader, soundId, repeatInstantly);
        }

        if (TrackData.isLocalSound(onlineSound.getURL())) {
            Identifier name = Identifier.tryParse(onlineSound.getURL());
            if (name != null) {
                WeighedSoundEvents weighedSoundEvents = Minecraft.getInstance().getSoundManager().getSoundEvent(name);
                if (weighedSoundEvents == null) {
                    return CompletableFuture.failedFuture(new FileNotFoundException("Unable to play unknown soundEvent: " + soundId));
                }
        ...
        return SoundCache.getAudioStream(onlineSound.getURL(), onlineSound.getProgressListener(), onlineSound.getAudioFileType()).thenCompose(AudioSource::openStream).thenApplyAsync(stream -> {
            ...
            try {
                ...
                // Try loading as OGG
                try {
                    return getStream(repeatInstantly ? new LoopingAudioStream(JOrbisAudioStream::new, is) : new JOrbisAudioStream(is), onlineSound);
                } catch (Exception e) {
                    is.reset();
                    ...
                    try {
                        ...
                        return getStream(repeatInstantly ? new LoopingAudioStream(input -> new RawAudioStream(format, input), ais) : new RawAudioStream(format, ais), onlineSound);
                    } catch (Exception e1) {
                        is.reset();
                        ...
                        try {
                            ...
                            return getStream(repeatInstantly ? new LoopingAudioStream(input -> new RawAudioStream(mp3InputStream.getFormat(), input), mp3InputStream) : new RawAudioStream(mp3InputStream.getFormat(), mp3InputStream), onlineSound);
                        } catch (Exception e2) {
