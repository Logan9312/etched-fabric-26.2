...
public interface SoundDownloadSource {
    ...
    static Map<String, String> getDownloadHeaders() {
        ...
        map.put("X-Minecraft-Version", version.name());
        map.put("X-Minecraft-Version-ID", version.id());
        map.put("X-Minecraft-Pack-Format", String.valueOf(version.packVersion(PackType.CLIENT_RESOURCES)));
        map.put("User-Agent", "Minecraft Java/" + version.name());
        return map;
    }
