    ...
    public static NativeImage apply(NativeImage image, NativeImage overlay) {
        ...
        for (int m = 0; m < overlay.getWidth(); ++m) {
            for (int n = 0; n < overlay.getHeight(); ++n) {
                ...
                int baseColor = alphaBlend(image.getPixel(x1, y1), image.getPixel(x2, y1), image.getPixel(x1, y2), image.getPixel(x2, y2));
                int overlayColor = overlay.getPixel(m, n);
                result.setPixel(m, n, (overlayColor & 0xFF000000) | multiply(baseColor, overlayColor, 16) | multiply(baseColor, overlayColor, 8) | multiply(baseColor, overlayColor, 0));
            }
        }
