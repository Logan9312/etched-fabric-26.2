...
public class AlbumCoverComponent implements TooltipProvider {
    ...
    public static Builder builder(ItemStack stack) {
        ...
        AlbumCoverComponent component = stack.get(EtchedComponents.ALBUM_COVER.get());
        if (component != null) {
    ...
    public static boolean isValid(ItemStack stack) {
        return PlayableRecord.isPlayableRecord(stack) && !stack.has(EtchedComponents.ALBUM_COVER.get());
    }
    ...
    public void addToTooltip(Item.TooltipContext context, Consumer<Component> tooltipAdder, TooltipFlag tooltipFlag, DataComponentGetter components) {
        for (ItemStack record : this.items) {
            if (!record.isEmpty()) {
                record.addToTooltip(DataComponents.JUKEBOX_PLAYABLE, context, record.getOrDefault(DataComponents.TOOLTIP_DISPLAY, TooltipDisplay.DEFAULT), tooltipAdder, tooltipFlag);
