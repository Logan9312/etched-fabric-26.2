...
public record PlayingRecordComponent(ItemStack stack) implements TooltipProvider {
    ...
    public void addToTooltip(Item.TooltipContext context, Consumer<Component> tooltipAdder, TooltipFlag tooltipFlag, DataComponentGetter components) {
        if (!this.stack.isEmpty()) {
            ...
            TooltipDisplay display = this.stack.getOrDefault(DataComponents.TOOLTIP_DISPLAY, TooltipDisplay.DEFAULT);
            this.stack.addToTooltip(EtchedComponents.ALBUM_COVER.get(), context, display, tooltipAdder, tooltipFlag);
            this.stack.addToTooltip(DataComponents.JUKEBOX_PLAYABLE, context, display, tooltipAdder, tooltipFlag);
