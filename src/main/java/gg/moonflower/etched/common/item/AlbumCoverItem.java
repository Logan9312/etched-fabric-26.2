...
public class AlbumCoverItem extends Item implements ContainerItem {
    ...
    public boolean overrideStackedOnOther(ItemStack stack, Slot slot, ClickAction clickAction, Player player) {
        ...
        if (clickItem.isEmpty()) {
            ...
            if (count != remaining.getCount()) {
                ...
                stack.set(EtchedComponents.ALBUM_COVER.get(), albumCover.build());
    ...
    public void onDestroyed(ItemEntity itemEntity) {
        AlbumCoverComponent albumCover = itemEntity.getItem().get(EtchedComponents.ALBUM_COVER.get());
        if (albumCover != null) {
            List<ItemStack> items = albumCover.getItems();
            AlbumCoverComponent.Builder builder = albumCover.toBuilder();
            builder.clearContent();
            itemEntity.getItem().set(EtchedComponents.ALBUM_COVER.get(), builder.build());
            ItemUtils.onContainerDestroyed(itemEntity, items.stream());
        }
    }
