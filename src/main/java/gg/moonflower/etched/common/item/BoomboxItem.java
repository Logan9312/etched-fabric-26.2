...
public class BoomboxItem extends Item implements ContainerItem {
    ...
    public static void onLivingEntityUpdateClient(LivingEntity entity) {
        ...
        if (entity instanceof Player player && Minecraft.getInstance().getCameraEntity() == entity) {
            ItemStack carried = player.inventoryMenu.getCarried();
            if (hasRecord(carried) && !carried.has(EtchedComponents.PAUSED.get())) {
        ...
        if (newPlayingRecord.isEmpty()) {
            for (InteractionHand hand : InteractionHand.values()) {
                ...
                if (hasRecord(stack) && !stack.has(EtchedComponents.PAUSED.get())) {
        ...
        if (entity instanceof Player player && newPlayingRecord.isEmpty() && Minecraft.getInstance().getCameraEntity() == entity) {
    ...
    public boolean onEntityItemUpdate(ItemStack stack, ItemEntity entity) {
        ...
        updatePlaying(entity, !stack.has(EtchedComponents.PAUSED.get()) ? getRecord(stack) : ItemStack.EMPTY);
        return false;
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (player.isSecondaryUseActive()) {
            if (stack.has(EtchedComponents.PAUSED.get())) {
                stack.remove(EtchedComponents.PAUSED.get());
            } else {
                stack.set(EtchedComponents.PAUSED.get(), PausedComponent.INSTANCE);
            }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, net.minecraft.world.item.component.TooltipDisplay display, java.util.function.Consumer<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        tooltipComponents.accept(PAUSE);
    }
    ...
    public static boolean hasRecord(ItemStack stack) {
        return stack.has(EtchedComponents.PLAYING_RECORD.get());
    }

    public static ItemStack getRecord(ItemStack stack) {
        PlayingRecordComponent record = stack.get(EtchedComponents.PLAYING_RECORD.get());
        return record != null ? record.stack() : ItemStack.EMPTY;
    }

    public static void setRecord(ItemStack stack, ItemStack record) {
        if (record.isEmpty()) {
            stack.remove(EtchedComponents.PLAYING_RECORD.get());
        } else {
            stack.set(EtchedComponents.PLAYING_RECORD.get(), new PlayingRecordComponent(record.copyWithCount(1)));
        }
    }
}
