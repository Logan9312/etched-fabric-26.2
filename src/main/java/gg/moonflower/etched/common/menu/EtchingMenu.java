...
public class EtchingMenu extends AbstractContainerMenu implements UrlMenu {
    ...
    public void slotsChanged(Container container) {
        ...
        if (resultStack.isEmpty() && labelStack.isEmpty()) {
            DiscAppearanceComponent discAppearance = discStack.get(EtchedComponents.DISC_APPEARANCE.get());
    ...
    private void setupResultSlot() {
        ...
        if (this.labelIndex.get() >= 0 && this.labelIndex.get() < DiscAppearanceComponent.LabelPattern.values().length) {
            ...
            if (discStack.is(EtchedItems.ETCHED_MUSIC_DISC.get()) || (!discStack.isEmpty() && !labelStack.isEmpty())) {
                ...
                MusicLabelComponent label = labelStack.get(EtchedComponents.MUSIC_LABEL.get());
                if (label != null) {
                ...
                this.currentRequest = CompletableFuture.supplyAsync(() -> {
                    ...
                    if (discStack.is(EtchedItems.ETCHED_MUSIC_DISC.get())) {
                        ...
                        resultStack.remove(EtchedComponents.ALBUM.get());
                    } else {
                    ...
                    DiscAppearanceComponent discAppearance = resultStack.get(EtchedComponents.DISC_APPEARANCE.get());
                    if (discAppearance != null) {
                    ...
                    if (SoundSourceManager.isValidUrl(this.url)) {
                        try {
                            ...
                            if (tracks.length == 1) {
                                ...
                                resultStack.set(EtchedComponents.ALBUM.get(), tracks[0]);
                        ...
                        try {
                            data[0] = data[0].withUrl(Identifier.parse(this.url).toString());
                    ...
                    resultStack.set(EtchedComponents.MUSIC.get(), new MusicTrackComponent(Arrays.asList(data)));
                    resultStack.set(EtchedComponents.DISC_APPEARANCE.get(), new DiscAppearanceComponent(DiscAppearanceComponent.LabelPattern.values()[this.labelIndex.get()], discColor, primaryLabelColor, secondaryLabelColor));
