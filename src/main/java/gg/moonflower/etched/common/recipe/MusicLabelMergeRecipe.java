...
public class MusicLabelMergeRecipe extends CustomRecipe {
    ...
    public boolean matches(CraftingInput input, Level level) {
        ...
        for (int i = 0; i < input.size(); ++i) {
            ...
            if (!stack.isEmpty()) {
                if (!stack.has(EtchedComponents.MUSIC_LABEL.get())) {
                    return false;
                }
                count++;
            }
    ...
    public ItemStack assemble(CraftingInput input) {
        ...
        MusicLabelComponent firstLabel = first.getOrDefault(EtchedComponents.MUSIC_LABEL.get(), MusicLabelComponent.DEFAULT);
        MusicLabelComponent secondLabel = second.getOrDefault(EtchedComponents.MUSIC_LABEL.get(), MusicLabelComponent.DEFAULT);
        ...
        stack.set(EtchedComponents.MUSIC_LABEL.get(), merge(firstLabel, secondLabel));
        return stack;
    }
